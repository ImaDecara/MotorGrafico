...
class HitBoxOBB(Hit):
    def _init_(self, get_model_matrix, hittable = True):
        super().__init__(get_model_matrix, hittable)
    ...
    def check_hit(self, origin, direction):
        ...
        local_origin = inv_model * glm.vec4(origin.x, origin.y, origin.z, 1.0)
        local_dir = inv_model * glm.vec4(direction.x, direction.y, direction.z, 0.0)
